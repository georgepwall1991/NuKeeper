...
public class GiteaSettingsReader : ISettingsReader
{
    ...
    public async Task<bool> CanRead(Uri repositoryUri)
    {
        ...
        try
        {
            ...
            var response = await client.GetAsync("swagger.v1.json").ConfigureAwait(false);
    ...
    public async Task<RepositorySettings> RepositorySettings(Uri repositoryUri, bool setAutoMerge,
    ...
    private static Task<RepositorySettings> CreateSettingsFromRemote(Uri repositoryUri, string targetBranch)
    {
        ...
        return InternalCreateRepositorySettings(apiUri, repositoryUri, repoName, repoOwner,
            targetBranch == null ? null : new RemoteInfo { BranchName = targetBranch });
    }
    ...
    private async Task<RepositorySettings> CreateSettingsFromLocal(Uri repositoryUri, string targetBranch)
    {
        ...
        if (await _gitDriver.IsGitRepo(repositoryUri))
        {
            ...
            if (origin != null)
            {
                var repo = await _gitDriver
                    .DiscoverRepo(repositoryUri); // Set to the folder, because we found a remote git repository
                ...
                remoteInfo.BranchName = targetBranch ??
                                        await _gitDriver.GetCurrentHead(remoteInfo.LocalRepositoryUri)
        ...
        return await InternalCreateRepositorySettings(remoteSettings.ApiUri, remoteSettings.RepositoryUri,
            remoteSettings.RepositoryName, remoteSettings.RepositoryOwner, remoteInfo).ConfigureAwait(false);
    }

    private static Task<RepositorySettings> InternalCreateRepositorySettings(Uri apiUri, Uri repositoryUri,
        string repoName, string repoOwner, RemoteInfo remoteInfo = null)
    {
