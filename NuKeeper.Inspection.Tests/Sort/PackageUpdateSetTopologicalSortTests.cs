...
public class PackageUpdateSetTopologicalSortTests
{
    ...
    public void CanSortTwoRelatedItemsinReverseOrder()
    {
        ...
        logger.DidNotReceive().Detailed("No dependencies between items, no need to sort on dependencies");
        logger.Received(1)
            .Detailed("Resorted 2 packages by dependencies, first change is fish moved to position 0 from 1.");
    }
    ...
    private static PackageUpdateSet MakeUpdateSet(string packageId, string packageVersion,
        PackageDependency upstream = null)
    {
        var currentPackages = new List<PackageInProject>
        {
            new(packageId, packageVersion, PathToProjectOne()),
            new(packageId, packageVersion, PathToProjectTwo())
        };
    ...
    private static PackageSearchMetadata Metadata(string packageId, string version, PackageDependency upstream)
    {
        ...
        if (upstream != null) upstreams.Add(upstream);
