...
[Command("repo", "r", "repository",
    Description = "Performs version checks and generates pull requests for a single repository.")]
internal class RepositoryCommand : CollaborationPlatformCommand
{
    ...
    public RepositoryCommand(ICollaborationEngine engine, IConfigureLogger logger, IFileSettingsCache fileSettingsCache,
        ICollaborationFactory collaborationFactory, IEnumerable<ISettingsReader> settingsReaders)
    ...
    [Option(CommandOptionType.NoValue, ShortName = "", LongName = "setautomerge",
        Description =
            "Set automatically auto merge for created pull request. Works only for Azure Devops. Defaults to false.")]
    ...
    [Option(CommandOptionType.SingleValue, LongName = "targetBranch",
        Description =
            "Use another target branch than the currently active HEAD branch of the remote or local repository")]
    public string TargetBranch { get; set; }

    [Option(CommandOptionType.SingleValue, ShortName = "cdir",
        Description =
            "If you want NuKeeper to check out the repository to an alternate path, set it here (by default, a temporary directory is used).")]
    ...
    {
        if (string.IsNullOrWhiteSpace(RepositoryUri)) return ValidationResult.Failure("Missing repository URI");
        ...
        var reader = await TryGetSettingsReader(repoUri, Platform).ConfigureAwait(false);

        if (reader == null)
            return ValidationResult.Failure(
                $"Unable to work out which platform to use {RepositoryUri} could not be matched");

        settings.SourceControlServerSettings.Repository = await reader
            .RepositorySettings(repoUri, SetAutoMerge ?? false, TargetBranch).ConfigureAwait(false);
    ...
    private async Task<ISettingsReader> TryGetSettingsReader(Uri repoUri, Platform? platform)
    {
        ...
        if (platform.HasValue) return _settingsReaders.Single(s => s.Platform == Platform);
