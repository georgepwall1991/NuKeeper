...
public class PackageUpdater : IPackageUpdater
{
    ...
    )
    {
        ...
        foreach (var updateSets in groups)
        {
            ...
            if (openPrs == allowedPrs) return (totalCount, true);
        }
        ...
        IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        if (!ditCheckOut) await git.CheckoutRemoteToLocal(branchWithChanges).ConfigureAwait(false);

        var filteredUpdates = await _existingCommitFilter
            .Filter(git, updates, repository.DefaultBranch, branchWithChanges).ConfigureAwait(false);
        ...
        if (haveUpdates)
        {
            ...
            var pullRequestExists = await _collaborationFactory.CollaborationPlatform
                .PullRequestExists(repository.Pull, qualifiedBranch, repository.DefaultBranch).ConfigureAwait(false);
            ...
            if (!pullRequestExists)
            {
                ...
                var pullRequestRequest = new PullRequestRequest(qualifiedBranch, title, repository.DefaultBranch,
                    ...
                    settings.SourceControlServerSettings.Repository.SetAutoMerge) { Body = body };
