...
public class PrioritySort : IPackageUpdateSetSort
{
    ...
    private static long Priority(PackageUpdateSet update)
    {
        ...
        var score = countCurrentVersions;
        score = score * Shift;
    ...
    private static long ScoreAge(PackageUpdateSet update)
    {
        ...
        if (!publishedDate.HasValue) return 0;
    ...
    private static long ScoreVersionChange(NuGetVersion newVersion, NuGetVersion oldVersion)
    {
        ...
        if (oldVersion.IsPrerelease && !newVersion.IsPrerelease) preReleaseScore = Shift * 12;

        var majors = newVersion.Major - oldVersion.Major;
        if (majors > 0) return majors * 100 + preReleaseScore;

        var minors = newVersion.Minor - oldVersion.Minor;
        if (minors > 0) return minors * 10 + preReleaseScore;

        var patches = newVersion.Patch - oldVersion.Patch;
        if (patches > 0) return patches + preReleaseScore;
