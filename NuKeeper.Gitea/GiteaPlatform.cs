...
public class GiteaPlatform : ICollaborationPlatform
{
    ...
    public async Task OpenPullRequest(ForkData target, PullRequestRequest request, IEnumerable<string> labels)
    {
        ...
        var mergeRequest = new CreatePullRequestOption
        {
            Title = request.Title,
            Head = request.Head,
            Body = request.Body,
    ...
    private static Organization MapOrganization(Model.Organization x)
    {
        return new Organization(x.FullName);
    }

    private Repository MapRepository(Model.Repository repo)
    {
