...
public class OrganisationCommandTests
{
    private static CollaborationFactory GetCollaborationFactory(
        Func<IGitDiscoveryDriver, IEnvironmentVariablesProvider, ISettingsReader> createSettingsReader)
    {
        ...
        return new CollaborationFactory(
            new[] { createSettingsReader(new MockedGitDiscoveryDriver(), environmentVariablesProvider) },
    ...
    public async Task CommandLineWillOverrideForkMode()
    {
        var (_, platformSettings) =
            await CaptureSettings(FileSettings.Empty(), false, false, null, ForkMode.PreferSingleRepository);
            ...
            ForkMode? forkMode = null)
    {
        ...
        if (addCommandRepoExclude) command.ExcludeRepos = "ExcludeFromCommand";

        if (forkMode != null) command.ForkMode = forkMode;
