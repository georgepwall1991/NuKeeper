...
public class AzureDevOpsSettingsReader : BaseSettingsReader
{
    ...
    private static RepositorySettings CreateSettingsFromRemote(Uri repositoryUri, string targetBranch)
    {
        ...
        if (indexOfGit == 1 && pathParts.Length == 3)
            return CreateRepositorySettings(
                null, //org
                repositoryUri, //uri
        ...
        if (await _gitDriver.IsGitRepo(repositoryUri))
        {
            ...
            if (origin != null)
            {
                remoteInfo.LocalRepositoryUri =
                    await _gitDriver
                        .DiscoverRepo(localCopy); // Set to the folder, because we found a remote git repository
    ...
    private static RepositorySettings CreateRepositorySettings(string org, Uri repositoryUri, string project,
        string repoName, RemoteInfo remoteInfo = null)
    {
        return new RepositorySettings
        {
            ApiUri = string.IsNullOrWhiteSpace(org)
