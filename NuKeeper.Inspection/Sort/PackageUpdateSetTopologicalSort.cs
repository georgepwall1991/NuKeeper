...
public class PackageUpdateSetTopologicalSort : IPackageUpdateSetSort
{
    ...
    private static IReadOnlyCollection<PackageUpdateSet> PackageDeps(PackageUpdateSet set,
    ...
    private void ReportSort(IList<PackageUpdateSet> input, IList<PackageUpdateSet> output)
    {
        ...
        for (var i = 0; i < output.Count; i++)
            if (input[i] != output[i])
            {
                ...
                _logger.Detailed(
                    $"Resorted {output.Count} packages by dependencies, first change is {firstChange.SelectedId} moved to position {i} from {originalIndex}.");
                break;
            }

        if (!hasChange) _logger.Detailed($"Sorted {output.Count} packages by dependencies but no change made");
    }
}
