...
public class ExistingCommitFilterTest : TestWithFailureLogging
{
    ...
    private IExistingCommitFilter MakeExistingCommitFilter()
    {
        ...
        commitWorder.MakeCommitMessage(Arg.Any<PackageUpdateSet>()).Returns(p =>
            $"Automatic update of {((PackageUpdateSet)p[0]).SelectedId} to {((PackageUpdateSet)p[0]).SelectedVersion}");
    ...
    private static Task<IReadOnlyCollection<string>> FixedReturnVal(string[] ids)
    {
        return Task.Run(() =>
        {
            return (IReadOnlyCollection<string>)ids.Select(id => CreateCommitMessage(id, new NuGetVersion("3.0.0")))
                .ToList().AsReadOnly();
        });
    }
    ...
    private static PackageUpdateSet MakeUpdateSet(string id)
    {
        ...
        {
            new(id, "1.0.0", new PackagePath("base", "rel", PackageReferenceType.ProjectFile)),
            new(id, "2.0.0", new PackagePath("base", "rel", PackageReferenceType.ProjectFile))
        };
