...
public class ExistingCommitFilter : IExistingCommitFilter
{
    ...
    public async Task<IReadOnlyCollection<PackageUpdateSet>> Filter(IGitDriver git,
        IReadOnlyCollection<PackageUpdateSet> updates, string baseBranch, string headBranch)
    {
        ...
        try
        {
            ...
            var compactCommitMessages =
                commitMessages.Select(m => new string(m.Where(c => !char.IsWhiteSpace(c)).ToArray()));

            foreach (var update in updates)
            {
                var updateCommitMessage = _collaborationFactory.CommitWorder.MakeCommitMessage(update);
                var compactUpdateCommitMessage =
                    new string(updateCommitMessage.Where(c => !char.IsWhiteSpace(c)).ToArray());

                if (!compactCommitMessages.Contains(compactUpdateCommitMessage)) filtered.Add(update);
            }
