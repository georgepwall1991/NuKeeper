...
public class RepositoryCommandTests
{
        ...
        )
    {
        ...
        if (addLabels) command.Label = new List<string> { "runLabel1", "runLabel2" };
    ...
    public async Task ShouldInitialisePlatformFromParameter(Platform platform, string expectedApi)
    {
        ...
        var gitHubSettingReader =
            new GitHubSettingsReader(new MockedGitDiscoveryDriver(), _environmentVariablesProvider);
        ...
        await collaborationFactory
            ...
            .Initialise(
                Arg.Is(new Uri(
                    expectedApi)), // Is populated by the settings reader. Thus, can be used to check if the correct one was selected.
                Arg.Is((string)null),
                Arg.Is((ForkMode?)null),
    ...
    public async Task UseCustomCheckoutDirectoryIfParameterIsProvidedForRemote()
    {
        ...
        collaborationFactorySubstitute.ForkFinder.FindPushFork(Arg.Any<string>(), Arg.Any<ForkData>())
            .Returns(Task.FromResult(new ForkData(testUri, "nukeeper", "nukeeper")));
        var folderFactorySubstitute = Substitute.For<IFolderFactory>();
        folderFactorySubstitute.FolderFromPath(Arg.Any<string>())
            .Returns(ci => new Folder(Substitute.For<INuKeeperLogger>(), new DirectoryInfo(ci.Arg<string>())));
        ...
        var gitEngine = new GitRepositoryEngine(updater, collaborationFactorySubstitute, folderFactorySubstitute,
            Substitute.For<INuKeeperLogger>(), Substitute.For<IRepositoryFilter>(), Substitute.For<ILogger>());
        ...
        await updater.Received().Run(Arg.Any<IGitDriver>(),
            ...
            Arg.Is<SettingsContainer>(c =>
                c.WorkingFolder.FullPath.EndsWith("testdirectory", StringComparison.Ordinal)));
    }
    ...
    public async Task UseCustomTargetBranchIfParameterIsProvided()
    {
        ...
        var updater = Substitute.For<IRepositoryUpdater>();
        var gitEngine = new GitRepositoryEngine(updater, collaborationFactorySubstitute,
            Substitute.For<IFolderFactory>(),
            Substitute.For<INuKeeperLogger>(), Substitute.For<IRepositoryFilter>(), Substitute.For<ILogger>());
    ...
    private static ICollaborationFactory GetCollaborationFactory(
        IEnvironmentVariablesProvider environmentVariablesProvider,
        IEnumerable<ISettingsReader> settingReaders = null)
    {
        return new CollaborationFactory(
            settingReaders ?? new ISettingsReader[]
                { new GitHubSettingsReader(new MockedGitDiscoveryDriver(), environmentVariablesProvider) },
