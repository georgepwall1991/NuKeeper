...
public class CollaborationEngine : ICollaborationEngine
{
    ...
    public async Task<int> Run(SettingsContainer settings)
    {
        ...
        var repositories = await _collaborationFactory.RepositoryDiscovery
            .GetRepositories(settings.SourceControlServerSettings).ConfigureAwait(false);
        ...
        foreach (var repository in repositories)
        {
            ...
            try
            {
                ...
                if (updatesInThisRepo > 0) reposUpdated++;
            }
...
#pragma warning restore CA1031
        ...
        if (reposUpdated > 1) _logger.Detailed($"{reposUpdated} repositories were updated");
    ...
    )
    {
        if (unhandledEx.Happened)
        {
            ...
            if (exception is AggregateException aggregateException) exception = aggregateException.Flatten();
