...
public class PackageUpdatesLookupTests
{
    ...
    [Test]
    public async Task
        FindUpdatesForPackages_OnePackageDifferentVersionsInDifferentProjects_RespectsAllowedChangeForEachVersionIndependently()
    {
        ...
        Assert.That(versionUpdates,
            Has.Some.Matches<string>(version => version.StartsWith("6.", StringComparison.OrdinalIgnoreCase)));
        Assert.That(versionUpdates,
            Has.Some.Matches<string>(version => version.StartsWith("10.", StringComparison.OrdinalIgnoreCase)));
    }
