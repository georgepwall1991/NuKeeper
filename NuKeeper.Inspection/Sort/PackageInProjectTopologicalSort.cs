    ...
    private void ReportSort(IList<PackageInProject> input, IList<PackageInProject> output)
    {
        ...
        for (var i = 0; i < output.Count; i++)
            if (input[i] != output[i])
            {
                ...
                _logger.Detailed(
                    $"Resorted {output.Count} projects by dependencies, first change is {firstChange.Path.RelativePath} moved to position {i} from {originalIndex}.");
                break;
            }

        if (!hasChange) _logger.Detailed($"Sorted {output.Count} projects by dependencies but no change made");
    }
}
