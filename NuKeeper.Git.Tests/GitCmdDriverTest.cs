...
public class GitCmdDriverTest
{
    ...
    {
        if (_pathToGit == null) Assert.Ignore("no git implementation found!");

        var folder = TestDirectoryHelper.UniqueTemporaryFolder();
        try
        {
            var gitDriver = new GitCmdDriver(_pathToGit, _logger, new Folder(_logger, folder),
                new GitUsernamePasswordCredentials());
            Assert.DoesNotThrowAsync(() => gitDriver.Clone(new Uri(path)));
            Assert.IsTrue(Directory.Exists(Path.Combine(folder.FullName, ".git")), "Local git repo should exist in {0}",
                folder.FullName);
    ...
    public async Task GetNewCommitMessages()
    {
        ...
        try
        {
            var creds = new GitUsernamePasswordCredentials();
            ...
            var master = branchNames.Where(b => b.EndsWith("/master", StringComparison.InvariantCultureIgnoreCase))
                .FirstOrDefault();
            if (master != null && branchNames.Length > 1)
            {
                ...
                Assert.AreEqual(origMessagesArray, cmdMessagesArray,
                    "GitCmdDriver does not return the right amount of messages");

                foreach (var message in origMessages)
                    Assert.IsTrue(cmdMessages.Contains(message),
                        $"GitCmdDriver does not return commit message {message}");
            }
        }
        finally
        {
