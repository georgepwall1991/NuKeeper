...
public class TopologicalSort<T>
{
    ...
    private readonly List<T> _sortedList = new();
    ...
    {
        if (inputMap == null) throw new ArgumentNullException(nameof(inputMap));

        var inputItems = inputMap
            .Select(i => i.Item)
            .ToList();

        if (inputMap.Count < 2) return inputItems;
    ...
    {
        if (_cycleFound) return;

        if (item.Mark == Mark.Permanent) return;
        ...
        foreach (var dep in NodesDependedOn(item)) Visit(dep);
