...
public class CollaborationFactory : ICollaborationFactory
{
    ...
    {
        var platformSettingsReader =
            await FindPlatformSettingsReader(platformFromSettings, apiEndpoint).ConfigureAwait(false);
        ...
        Platform? platformFromSettings, Uri apiEndpoint)
    {
        if (platformFromSettings.HasValue)
        {
            ...
            if (reader != null) _nuKeeperLogger.Normal($"Collaboration platform specified as '{reader.Platform}'");
    ...
    private ValidationResult ValidateSettings()
    {
        ...
        if (string.IsNullOrWhiteSpace(Settings.Token)) return ValidationResult.Failure("Token was not set");

        if (!_platform.HasValue) return ValidationResult.Failure("Platform was not set");
    ...
    private void CreateForPlatform()
    {
        ...
        switch (_platform.Value)
        {
            case Platform.AzureDevOps:
                ...
                RepositoryDiscovery =
                    new AzureDevOpsRepositoryDiscovery(_nuKeeperLogger, CollaborationPlatform, Settings.Token);
            ...
            case Platform.BitbucketLocal:
                ...
                RepositoryDiscovery =
                    new BitbucketLocalRepositoryDiscovery(_nuKeeperLogger, CollaborationPlatform, Settings);
                ForkFinder = new BitbucketForkFinder(CollaborationPlatform, _nuKeeperLogger, forkMode);
                CommitWorder = new DefaultCommitWorder();
                break;
