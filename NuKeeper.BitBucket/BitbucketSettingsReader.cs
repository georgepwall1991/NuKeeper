...
public class BitbucketSettingsReader : ISettingsReader
{
    ...
    public Task<bool> CanRead(Uri repositoryUri)
    {
        return Task.FromResult(
            repositoryUri?.Host.Contains("bitbucket.org", StringComparison.OrdinalIgnoreCase) == true);
    }
    ...
    {
        if (repositoryUri == null) return Task.FromResult<RepositorySettings>(null);
        ...
        if (pathParts.Count != 2)
            throw new NuKeeperException(
                $"The provided uri was is not in the correct format. Provided {repositoryUri} and format should be https://username_@bitbucket.org/projectname/repositoryname.git");
