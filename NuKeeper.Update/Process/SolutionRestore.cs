...
public class SolutionRestore : ISolutionRestore
{
    ...
    public async Task CheckRestore(IEnumerable<PackageUpdateSet> targetUpdates, IFolder workingFolder,
        NuGetSources sources)
    {
        if (workingFolder == null) throw new ArgumentNullException(nameof(workingFolder));

        if (AnyProjectRequiresNuGetRestore(targetUpdates)) await Restore(workingFolder, sources).ConfigureAwait(false);
    }
    ...
    private async Task Restore(IFolder workingFolder, NuGetSources sources)
    {
        ...
        foreach (var sln in solutionFiles) await _fileRestoreCommand.Invoke(sln, sources).ConfigureAwait(false);
    }
