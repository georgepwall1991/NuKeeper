...
public static class TimeSpanFormat
{
    public static string Ago(DateTime start, DateTime end)
    {
        ...
        if (start.Year == end.Year && start.Month == end.Month) return Ago(duration);
        ...
        if (years == 0) return Plural(remainderMonth, "month") + " ago";

        if (remainderMonth == 0) return Plural(years, "year") + " ago";
    ...
    {
        if (start.Year == end.Year) return end.Month - start.Month;
    ...
    {
        if (ago.TotalSeconds < 1) return "now";
