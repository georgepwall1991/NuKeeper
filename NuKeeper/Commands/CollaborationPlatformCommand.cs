...
internal abstract class CollaborationPlatformCommand : CommandBase
{
    ...
    private readonly HashSet<Platform> _platformsSupportingDeleteBranchAfterMerge = new();
    ...
    [Option(CommandOptionType.SingleValue, ShortName = "", LongName = "maxopenpullrequests",
        Description =
            "The maximum number of open pull requests for one repository. Defaults to 1 if `--consolidate` is specified, otherwise defaults to `--maxpackageupdates`.")]
    ...
    protected override async Task<ValidationResult> PopulateSettings(SettingsContainer settings)
    {
        ...
        var endpoint = Concat.FirstValue(ApiEndpoint, fileSettings.Api,
            settings.SourceControlServerSettings.Repository?.ApiUri.ToString());
        ...
        try
        {
            ...
            if (!collaborationResult.IsSuccess) return collaborationResult;
        }
...
#pragma warning restore CA1031
        ...
        if (!deleteBranchAfterMergeValid.IsSuccess) return deleteBranchAfterMergeValid;
        ...
        SettingsContainer settings)
    {
        ...
        settings.BranchSettings.DeleteBranchAfterMerge = Concat.FirstValue(DeleteBranchAfterMerge,
            fileSettings.DeleteBranchAfterMerge, defaultValue);
