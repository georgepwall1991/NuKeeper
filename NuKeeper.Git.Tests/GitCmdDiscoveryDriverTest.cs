...
public class GitCmdDiscoveryDriverTest
{
    ...
    public async Task ShouldGetRemotes()
    {
        ...
        Assert.AreEqual(remotesArray?.Length, classicRemotesArray?.Length,
            "Lib2Sharp and GitCmd should have the same number of results");

        for (var count = 0; count < classicRemotesArray.Length; count++)
        {
            var classicRemote = classicRemotesArray[count];
            var remote = remotesArray
                .Where(r => r.Name.Equals(classicRemote.Name, StringComparison.InvariantCultureIgnoreCase))
                .FirstOrDefault();
