...
public class UpdateDirectoryBuildTargetsCommand : IUpdateDirectoryBuildTargetsCommand
{
    ...
    private static IEnumerable<XElement> IncludesOrUpdates(PackageInProject currentPackage,
        IEnumerable<XElement> elements)
    {
        return elements.Where(el =>
            el.Attributes("Include")
                .Any(a => a.Value.Equals(currentPackage.Id, StringComparison.InvariantCultureIgnoreCase))
            || el.Attributes("Update")
                .Any(a => a.Value.Equals(currentPackage.Id, StringComparison.InvariantCultureIgnoreCase))
        );
    }
}
