...
public class AzureDevOpsCommitWorder : ICommitWorder
{
    ...
    public string MakePullRequestTitle(IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        if (updates.Count == 1) return PackageTitle(updates.First());
    ...
    public string MakeCommitDetails(IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        if (updates.Count > 1) MultiPackage(updates, builder);

        foreach (var update in updates) builder.AppendLine(MakeCommitVersionDetails(update));
    ...
    private static void MultiPackage(IReadOnlyCollection<PackageUpdateSet> updates, StringBuilder builder)
    {
        ...
        var projects = updates.SelectMany(u => u.CurrentPackages)
        ...
        var updatedPackageNames = "|";
        foreach (var packageName in packageNames) updatedPackageNames += $" {packageName} |";
    ...
    private static string MakeCommitVersionDetails(PackageUpdateSet updates)
    {
        ...
        {
            builder.AppendLine(
                $"NuKeeper has generated a {changeLevel} update of {packageId} to {newVersion} from {oldVersions.JoinWithCommas()}");
        }
        else
        {
            builder.AppendLine($"NuKeeper has generated a {changeLevel} update of {packageId} to {newVersion}");
            builder.AppendLine(
                $"{oldVersions.Count} versions of {packageId} were found in use: {oldVersions.JoinWithCommas()}");
        }
        ...
        foreach (var current in updates.CurrentPackages)
        {
            ...
            {
                line =
                    $"| {CodeQuote(current.Path.RelativePath)} | {CodeQuote(updates.SelectedId)} | {NuGetVersionPackageLink(current.Identity)} | {NuGetVersionPackageLink(updates.Selected.Identity)} |";
            ...
            line =
                $"| {CodeQuote(current.Path.RelativePath)} | {CodeQuote(updates.SelectedId)} | {current.Version.ToString()} | {updates.SelectedVersion.ToString()} |";
            builder.AppendLine(line);
        }
    ...
    private static string ChangeLevel(NuGetVersion oldVersion, NuGetVersion newVersion)
    {
        ...
        if (!newVersion.IsPrerelease && oldVersion.IsPrerelease) return "out of beta";
    ...
    {
        if (!highestPublishedAt.HasValue) return string.Empty;
