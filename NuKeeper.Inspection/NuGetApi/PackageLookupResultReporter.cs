...
public class PackageLookupResultReporter : IPackageLookupResultReporter
{
    ...
    public void Report(PackageLookupResult lookupResult)
    {
        ...
        if (highestVersion > highestMatchVersion)
            _logger.Normal(
                $"Selected update of package {packageId} to version {highestMatchVersion}, but version {highestVersion} is also available.{allowing}");
        else
            _logger.Detailed(
                $"Selected update of package {packageId} to highest version, {highestMatchVersion}.{allowing}");
    }
}
