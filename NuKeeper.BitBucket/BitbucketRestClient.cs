...
public class BitbucketRestClient
{
    ...
    {
        var response = await GetResource<IteratorBasedPage<ProjectInfo>>($"teams/{account}/projects/")
    ...
    {
        var response = await GetResource<IteratorBasedPage<Repository>>($"repositories/{account}")
            .ConfigureAwait(false);
        return response.values;
    }
    ...
    {
        var response = await GetResource<IteratorBasedPage<Ref>>($"repositories/{account}/{repositoryId}/refs")
            .ConfigureAwait(false);
        return response.values;
    }
    ...
    {
        var filter =
            $"state =\"open\" AND source.branch.name = \"{headBranch}\" AND destination.branch.name = \"{baseBranch}\"";

        return await GetResource<PullRequestsInfo>(
                $"repositories/{account}/{repositoryName}/pullrequests?q={HttpUtility.UrlEncode(filter)}")
            .ConfigureAwait(false);
    }
        ...
        [CallerMemberName] string caller = null)
    {
        var content = new StringContent(JsonSerializer.Serialize(request, JsonOptions), Encoding.UTF8,
            "application/json");
        var response = await _client.PostAsync($"repositories/{account}/{repositoryName}/pullrequests", content)
            .ConfigureAwait(false);
