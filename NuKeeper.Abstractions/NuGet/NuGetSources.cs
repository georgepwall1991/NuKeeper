...
public class NuGetSources
{
    ...
    {
        if (!sources.Any()) throw new ArgumentException("At least one source must be specified", nameof(sources));
    ...
    public NuGetSources(IEnumerable<PackageSource> sources)
    {
        ...
        if (!items.Any()) throw new ArgumentException("No package sources defined", nameof(sources));
