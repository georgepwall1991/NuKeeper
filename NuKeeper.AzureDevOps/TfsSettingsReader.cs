...
public class TfsSettingsReader : BaseSettingsReader
{
    ...
    private static RepositorySettings RepositorySettings(Uri repositoryUri, RemoteInfo remoteInfo = null)
    {
        ...
        if (gitLocation == -1)
            throw new NuKeeperException(
                "Unknown format. Format should be http(s)://tfs.company.local:port/<nothingOrVirtualSite>/{project}/_git/{repo}");
        ...
        {
            ApiUri = new Uri(
                $"{repositoryUri.Scheme}://{repositoryUri.Host}:{repositoryUri.Port}/{apiPathParts.JoinWithSeparator("/")}"),
