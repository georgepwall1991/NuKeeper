...
public static class TestDirectoryHelper
{
    ...
    public static void DeleteDirectory(DirectoryInfo toDelete)
    {
        ...
        DeleteDirectory(toDelete, 5, 16, 2);
    }
    ...
    private static void NormalizeAttributes(DirectoryInfo toNormalize)
    {
        ...
        foreach (var file in files) File.SetAttributes(file.FullName, FileAttributes.Normal);

        foreach (var subdirectory in subdirectories) NormalizeAttributes(subdirectory);
        ...
        for (var attempt = 1; attempt <= maxAttempts; attempt++)
            ...
            catch (Exception ex)
            {
                ...
                if (!whitelist.Any(knownExceptionType =>
                        knownExceptionType.IsAssignableFrom(caughtExceptionType))) throw;
