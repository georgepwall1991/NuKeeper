...
public class PackageVersionsLookup : IPackageVersionsLookup
{
    ...
    private readonly ConcurrentSourceRepositoryCache _packageSources = new();
        ...
        string packageName, bool includePrerelease, PackageSource source)
    {
        ...
        try
        {
            var metadataResource =
                await sourceRepository.GetResourceAsync<PackageMetadataResource>().ConfigureAwait(false);
            var metadatas = await FindPackage(metadataResource, packageName, includePrerelease).ConfigureAwait(false);
