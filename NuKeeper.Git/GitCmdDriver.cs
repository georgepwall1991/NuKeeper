...
public class GitCmdDriver : IGitDriver
{
    ...
    public async Task Clone(Uri pullEndpoint, string branchName)
    {
        ...
        await StartGitProcess($"clone{branchparam} {CreateCredentialsUri(pullEndpoint, _gitCredentials)} .",
    ...
    public async Task<string> GetCurrentHead()
    {
        ...
        return string.IsNullOrEmpty(getBranchHead) ? await StartGitProcess("rev-parse HEAD", true) : getBranchHead;
    }
    ...
    {
        var commitlog =
            await StartGitProcess($"log --oneline --no-decorate --right-only {baseBranchName}...{headBranchName}", true)
    ...
    {
        if (_gitCredentials?.Username == null) return pullEndpoint;
