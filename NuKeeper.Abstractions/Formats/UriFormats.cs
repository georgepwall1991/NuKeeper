...
public static class UriFormats
{
    public static Uri EnsureTrailingSlash(Uri uri)
    {
        ...
        if (path.EndsWith("/", StringComparison.OrdinalIgnoreCase)) return uri;

        return new Uri(path + "/");
    }

    public static async Task<Uri> GetRemoteUriFromLocalRepo(this Uri repositoryUri, IGitDiscoveryDriver discoveryDriver,
        string shouldMatchTo)
    {
        if (discoveryDriver == null) throw new ArgumentNullException(nameof(discoveryDriver));
    ...
    public static Uri ToUri(this string repositoryString)
    {
        ...
        if (!Directory.Exists(absoluteUri)) throw new NuKeeperException($"Local uri doesn't exist: {absoluteUri}");
