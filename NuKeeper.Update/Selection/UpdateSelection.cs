...
public class UpdateSelection : IUpdateSelection
{
        ...
        FilterSettings settings)
    {
        ...
        if (settings.MinimumAge != TimeSpan.Zero) _maxPublishedDate = DateTime.UtcNow.Subtract(settings.MinimumAge);
    ...
    private void LogPackageCounts(int candidates, int filtered, int capped)
    {
        ...
        if (filtered < candidates) message += $", filtered to {filtered}";

        if (capped < filtered) message += $", capped at {capped}";
    ...
    {
        if (!_maxPublishedDate.HasValue) return true;

        var published = packageUpdateSet.Selected.Published;
        if (!published.HasValue) return true;
