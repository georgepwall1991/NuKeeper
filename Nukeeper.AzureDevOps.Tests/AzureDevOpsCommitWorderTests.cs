...
public class AzureDevOpsCommitWorderTests
{
    ...
    public void OneUpdate_MakeCommitDetails_HasVersionInfo()
    {
        ...
        Assert.That(report,
            Does.StartWith("NuKeeper has generated a minor update of `foo.bar` to `1.2.3` from `1.1.0`"));
    }
    ...
    public void OneUpdate_MakeCommitDetails_HasProjectDetailsAsMarkdownTable()
    {
        ...
        Assert.That(report,
            Does.Contain(
                $"| `folder\\src\\project1\\packages.config` | `foo.bar` | {NuGetVersionPackageLink("foo.bar", "1.1.0")} | {NuGetVersionPackageLink("foo.bar", "1.2.3")} |"));
    }
    ...
    public void TwoUpdates_MakeCommitDetails_HasProjectList()
    {
        ...
        Assert.That(report,
            Does.Contain(
                $"| `folder\\src\\project2\\packages.config` | `foo.bar` | {NuGetVersionPackageLink("foo.bar", "1.0.0")} | {NuGetVersionPackageLink("foo.bar", "1.2.3")} |"));
    }
    ...
    public void TwoUpdatesSameVersion_MakeCommitDetails_HasProjectList()
    {
        ...
        Assert.That(report,
            Does.Contain(
                $"| `folder\\src\\project3\\packages.config` | `foo.bar` | {NuGetVersionPackageLink("foo.bar", "1.1.0")} | {NuGetVersionPackageLink("foo.bar", "1.2.3")} |"));
    }
    ...
    public void OneUpdateWithDate_MakeCommitDetails_HasVersionLimitDataWithDate()
    {
        ...
        Assert.That(report,
            Does.Contain("There is also a higher version, `foo.bar 2.3.4` published at `2018-02-20T11:32:45Z`,"));
        Assert.That(report,
            Does.Contain(" ago, but this was not applied as only `Minor` version changes are allowed."));
    }
    ...
    public void OneUpdateWithMajorVersionChange()
    {
        var updates = PackageUpdates
            .ForNewVersion(new PackageIdentity("foo.bar", new NuGetVersion("2.1.1")), MakePackageForV110())
            .InList();

        var report = _sut.MakeCommitDetails(updates);

        Assert.That(report,
            Does.StartWith("NuKeeper has generated a major update of `foo.bar` to `2.1.1` from `1.1.0"));
    }
    ...
    {
        var updates = PackageUpdates
            .ForNewVersion(new PackageIdentity("foo.bar", new NuGetVersion("1.2.1")), MakePackageForV110())
            .InList();

        var report = _sut.MakeCommitDetails(updates);

        Assert.That(report,
            Does.StartWith("NuKeeper has generated a minor update of `foo.bar` to `1.2.1` from `1.1.0"));
    }
    ...
    {
        var updates = PackageUpdates
            .ForNewVersion(new PackageIdentity("foo.bar", new NuGetVersion("1.1.9")), MakePackageForV110())
            .InList();

        var report = _sut.MakeCommitDetails(updates);

        Assert.That(report,
            Does.StartWith("NuKeeper has generated a patch update of `foo.bar` to `1.1.9` from `1.1.0"));
    }
    ...
    public void TwoUpdateSets()
    {
        ...
        {
            PackageUpdates.ForNewVersion(new PackageIdentity("foo.bar", new NuGetVersion("2.1.1")),
                MakePackageForV110()),
        ...
        Assert.That(report,
            Does.Contain("NuKeeper has generated a major update of `packageTwo` to `3.4.5` from `1.1.0`"));
    }
    ...
    private static void AssertContainsStandardText(string report)
    {
        ...
        Assert.That(report,
            Does.EndWith("**NuKeeper**: https://github.com/NuKeeperDotNet/NuKeeper" + Environment.NewLine));
    ...
    {
        var path = new PackagePath("c:\\temp", "folder\\src\\project3\\packages.config",
            PackageReferenceType.PackagesConfig);
