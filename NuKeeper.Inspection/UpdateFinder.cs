...
public class UpdateFinder : IUpdateFinder
{
    ...
    private IReadOnlyCollection<PackageInProject> FilteredByIncludeExclude(IReadOnlyCollection<PackageInProject> all,
        Regex includes, Regex excludes)
    {
        ...
        if (filteredByIncludeExclude.Count < all.Count)
        {
            ...
            if (excludes != null) filterDesc += $"Exclude '{excludes}'";

            if (includes != null) filterDesc += $"Include '{includes}'";
    ...
    private IReadOnlyCollection<PackageInProject> FindPackages(IFolder workingFolder)
    {
        ...
        foreach (var metaPackage in metaPackages) LogVersionedMetapackage(metaPackage);
    ...
    {
        _logger.Error(
            $"Metapackage '{metaPackage.Id}' has version {metaPackage.Version} in {metaPackage.Path.FullName}, " +
