...
public class LibGit2SharpDiscoveryDriver : IGitDiscoveryDriver
{
    ...
    public Task<Uri> DiscoverRepo(Uri repositoryUri)
    {
        return Task.Run(() =>
        {
            ...
            if (string.IsNullOrEmpty(discovery)) return null;
    ...
    public async Task<string> GetCurrentHead(Uri repositoryUri)
    {
        ...
        {
            var repoHeadBranch = repo.Branches.SingleOrDefault(b => b.IsCurrentRepositoryHead);

            if (repoHeadBranch == null)
                throw new NuKeeperException(
                    $"Cannot find current head branch for repo at '{repoRoot}', with {repo.Branches.Count()} branches");
