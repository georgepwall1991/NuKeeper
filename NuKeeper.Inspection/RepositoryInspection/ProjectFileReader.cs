...
public class ProjectFileReader : IPackageReferenceFinder
{
    ...
    public IReadOnlyCollection<PackageInProject> Read(Stream fileContents, string baseDirectory, string relativePath)
    {
        ...
        if (project == null) return Array.Empty<PackageInProject>();
        ...
        projectFileResults.AddRange(
            itemGroups
                ...
                .Select(el => XmlToPackage(ns, el,
                    new PackagePath(baseDirectory, relativePath, PackageReferenceType.DirectoryBuildTargets), null))
                .Where(el => el != null)
        );
