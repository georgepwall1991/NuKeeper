...
internal abstract class CommandBase
{
    ...
    private SettingsContainer MakeSettings()
    {
        ...
        var usePrerelease = Concat.FirstValue(UsePrerelease, fileSettings.UsePrerelease,
            Abstractions.Configuration.UsePrerelease.FromPrerelease);
    ...
    protected virtual async Task<ValidationResult> PopulateSettings(SettingsContainer settings)
    {
        ...
        if (!minPackageAge.HasValue)
            return await Task.FromResult(
                ValidationResult.Failure($"Min package age '{MinimumPackageAge}' could not be parsed"));

        settings.PackageFilters.MinimumAge = minPackageAge.Value;

        var regexIncludeValid = PopulatePackageIncludes(settings);
        if (!regexIncludeValid.IsSuccess) return regexIncludeValid;

        var regexExcludeValid = PopulatePackageExcludes(settings);
        if (!regexExcludeValid.IsSuccess) return regexExcludeValid;
        ...
        if (!branchNameTemplateValid.IsSuccess) return branchNameTemplateValid;
        ...
        SettingsContainer settings)
    {
        ...
        foreach (var token in BranchNamer.TemplateTokens) tokenValues.Add(token, "dummy");

        var validationValue = BranchNamer.MakeName(tokenValues, value);
        if (!Regex.IsMatch(validationValue,
