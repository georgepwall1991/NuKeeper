...
public class GitRepositoryEngine : IGitRepositoryEngine
{
        ...
        SettingsContainer settings, User user)
    {
        ...
        if (repositoryData == null) return 0;
        ...
        {
            var remoteRepoContainsDotNet =
                await _repositoryFilter.ContainsDotNetProjects(repository).ConfigureAwait(false);
        ...
        {
            folder = new Folder(_logger,
                new DirectoryInfo(Uri.UnescapeDataString(repository.RemoteInfo.LocalRepositoryUri.AbsolutePath)));
            settings.WorkingFolder = new Folder(_logger,
                new DirectoryInfo(Uri.UnescapeDataString(repository.RemoteInfo.WorkingFolder.AbsolutePath)));
        ...
        repositoryData.IsLocalRepo = repository.IsLocalRepo;
        var git = string.IsNullOrWhiteSpace(settings?.UserSettings?.GitPath)
        ...
        {
            if (!repository.IsLocalRepo) folder.TryDelete();
        }
    }
