...
public class AzureDevOpsForkFinderTests
{
    ...
    public async Task ThrowsWhenNoPushableForkCanBeFound()
    {
        ...
        var forkFinder = new AzureDevOpsForkFinder(Substitute.For<ICollaborationPlatform>(),
            Substitute.For<INuKeeperLogger>(), ForkMode.SingleRepositoryOnly);
    ...
    public void ThrowsWhenPreferFork()
    {
        ...
        var argument = Assert.Throws<ArgumentOutOfRangeException>(() =>
            new AzureDevOpsForkFinder(Substitute.For<ICollaborationPlatform>(), Substitute.For<INuKeeperLogger>(),
                ForkMode.PreferFork));
    }
    ...
    public async Task FallbackForkIsUsedWhenItIsFound()
    {
        ...
        var forkFinder = new AzureDevOpsForkFinder(collaborationPlatform, Substitute.For<INuKeeperLogger>(),
