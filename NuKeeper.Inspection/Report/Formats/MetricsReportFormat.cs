...
public class MetricsReportFormat : IReportFormat
{
    ...
    private void WriteMajorMinorPatchCount(IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        foreach (var update in updates)
        {
            ...
            if (majorUpdate != null && majorUpdate.Identity.Version.Major > baselineVersion.Major) majors++;
            if (minorUpdate != null && minorUpdate.Identity.Version.Minor > baselineVersion.Minor) minors++;

            if (patchUpdate != null) patches++;
        }
    ...
    private static PackageSearchMetadata FilteredPackageVersion(NuGetVersion baseline,
        PackageSearchMetadata packageVersion)
    {
        if (packageVersion == null) return null;

        if (packageVersion.Identity.Version <= baseline) return null;
