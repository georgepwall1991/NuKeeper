...
public static class UpdatesLogger
{
    public static string OldVersionsToBeUpdated(IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        if (updates.Count == 1) return $"Updating {DescribeOldVersions(updates.First())}";
    ...
    private static string DescribeOldVersions(PackageUpdateSet updateSet)
    {
        ...
        return
            $"'{updateSet.SelectedId}' from {oldVersions.JoinWithCommas()} to {updateSet.SelectedVersion} in {updateSet.CurrentPackages.Count} projects";
    }
}
