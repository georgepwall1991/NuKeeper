...
public class DefaultCommitWorder : ICommitWorder
{
    ...
    public string MakeCommitDetails(IReadOnlyCollection<PackageUpdateSet> updates)
    {
        ...
        if (updates.Count > 1) MultiPackagePrefix(updates, builder);

        foreach (var update in updates) builder.AppendLine(MakeCommitVersionDetails(update));

        if (updates.Count > 1) MultiPackageFooter(builder);
    ...
    private static string MakeCommitVersionDetails(PackageUpdateSet updates)
    {
        ...
        {
            var line =
                $"Updated {CodeQuote(current.Path.RelativePath)} to {packageId} {CodeQuote(updates.SelectedVersion.ToString())} from {CodeQuote(current.Version.ToString())}";
